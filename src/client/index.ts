...
import { clientMcp } from "../config/mcp";
...
async function main() {
  ...
  const [{ tools }, { prompts }, { resources }, { resourceTemplates }] =
    await Promise.all([
      clientMcp.listTools(),
      clientMcp.listPrompts(),
      clientMcp.listResources(),
      clientMcp.listResourceTemplates(),
    ]);

  clientMcp.setRequestHandler(CreateMessageRequestSchema, async (request) => {
    const texts: string[] = [];
...
async function handleQuery(tools: Tool[]) {
  ...
  const resultFromTools = (() => {
    if (!Array.isArray(toolResults)) return undefined;
    for (const tr of toolResults) {
      const res: any = tr; // cast to any to avoid 'never' type errors
      const textFromResult =
        res?.result?.content?.[0]?.text ?? res?.result?.content?.text;
      if (textFromResult) return textFromResult;
  ...
  console.log(text ?? resultFromTools ?? "No text generated.");
}
...
async function handleTool(tool: Tool) {
  ...
  const res = await clientMcp.callTool({
    name: tool.name,
    arguments: args,
  });
...
async function handleResource(uri: string) {
  ...
  const res = await clientMcp.readResource({
    uri: finalUri,
  });
...
async function handlePrompt(prompt: Prompt) {
  ...
  const response = await clientMcp.getPrompt({
    name: prompt.name,
    arguments: args,
  });
